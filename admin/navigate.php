...
$categorylist = $categorydata->GetCategoryList(0);
$categorylist2 = $categorydata->GetCategoryList(0,'product');
...
if($action == "update")
{
	...
	if(count($idarr) > 0)
	{
		foreach($idarr as $id)
		{
			$name=$_POST["name"];
			$url=$_POST["url"];
			$url[$id] = str_replace(siteurl(),"{siteurl}",$url[$id]);
			$catid = $_POST["catid"];
			$order=$_POST["order"];
			$group=$_POST["group"];
			if(is_numeric($id))
			{
				$sql = "update yiqi_navigate set `name`='".$name[$id]."',`url`='".$url[$id]."',`catid`='".intval($catid[$id])."',`displayorder`='".$order[$id]."',`group`='".$group[$id]."' WHERE `navid` = '$id' limit 1";
...
else if($action == "insert")
{
	...
	$sql = "INSERT INTO yiqi_navigate (`navid`,`name`,`url`,`catid`,`group`,`displayorder`,`status`) VALUES (null,'$navname','$navurl','$catid','$navgroup','$navorder','ok');";
...
<tr style="background:#f6f6f6;"><td class="w10">选择</td><td class="w20">链接标题</td><td class="w40">链接地址/栏目ID</td><td class="w10">排序</td><td class="w20">所属分组</td></tr>
<?php
if(count($navlist)>0)
{
    foreach($navlist as $navinfo)
    {
	    if($navinfo->catid){
			echo "<tr>".
				...
				"<td><input type=\"text\" class=\"txt\" style=\"width:260px;\" name=\"catid[$navinfo->navid]\" value=\"$navinfo->catid\" /></td>".
...
<h3>添加导航</h3>
<table class="inputform fl" cellpadding="1" cellspacing="1">
<tr><td colspan=2 align="center">添加分类（选择分类后，下面的自定义链接栏位不用填写。）</td></tr>
<tr><td class="label">选择分类</td><td class="input">
	<select name="catid">
...
<tr><td colspan=2 align="center">或者 自定义链接</td></tr>
...
<tr><td class="label">所属分组</td><td class="input"><input type="text" class="txt" name="navgroup" placeholder="顶部导航/次导航" /></td></tr>
</table>
<div class="clear">&nbsp;</div>
